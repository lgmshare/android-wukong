 ...
 */
package com.lgmshare.component.widget.viewflow;
...
public class ViewFlow extends AdapterView<Adapter> {
    ...
    private final static int TOUCH_STATE_MOVED = 2;
    ...
    public ViewFlow(Context context, AttributeSet attrs) {
        ...
        mSideBuffer = styledAttrs.getInt(R.styleable.ViewFlow_vfSideBuffer, 3);
        init();
    }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                if (mTouchState == TOUCH_STATE_SCROLLING) {
                    ...
                    if (mValidCount != 0 && mTouchState == TOUCH_STATE_REST) {
                        performItemClick(mLoadedViews.get(mCurrentBufferIndex),
                                mCurrentAdapterIndex % mValidCount, 0);
                    }
                }
    ...
    protected void onScrollChanged(int h, int v, int oldh, int oldv) {
        ...
        if (mIndicator != null) {
			 ...
			 */
            int hPerceived = h + (mCurrentAdapterIndex % mValidCount - mCurrentBufferIndex) * getChildWidth();
    ...
    private void postViewSwitched(int direction) {
        ...
        if (mIndicator != null) {
            mIndicator.onSwitched(mLoadedViews.get(mCurrentBufferIndex), mCurrentAdapterIndex % mValidCount);
        }
        if (mViewSwitchListener != null) {
            mViewSwitchListener.onSwitched(mLoadedViews.get(mCurrentBufferIndex), mCurrentAdapterIndex % mValidCount);
        }
        logBuffer();
    }
