...
/**
 * 提供给adapter绑定view的Holder
 *
 * @author: lim
 * @description: 提供给adapter绑定view的Holder
...
public class ViewHolder {
    ...
    private ViewHolder(Context context, ViewGroup parent, int resource, int position) {
        this.mContext = context;
        this.mDataPosition = position;
        this.mViews = new SparseArray<View>();
        this.mConvertView = LayoutInflater.from(context).inflate(resource, parent, false);
     ...
     * @return
     */
    public static ViewHolder get(Context context, View convertView, ViewGroup parent, int resource, int position) {
        ViewHolder holder;
        if (convertView == null) {
            holder = new ViewHolder(context, parent, resource, position);
        } else {
     ...
     * Picasso manages recycling of mViews in a ListView.<br/>
    ...
    public ViewHolder setImageUrl(int viewId, String imageUrl) {
        ImageView view = getView(viewId);
        Glide.with(mContext).load(imageUrl).crossFade().into(view);
        return this;
    }

    public ViewHolder setImageUrl(int viewId, String imageUrl, int defResourceId) {
        ImageView view = getView(viewId);
        Glide.with(mContext).load(imageUrl).crossFade().placeholder(defResourceId).into(view);
        return this;
    }

    public ViewHolder setImageUrl(int viewId, String imageUrl, int defResourceId, BitmapTransformation... transformations) {
        ImageView view = getView(viewId);
        Glide.with(mContext).load(imageUrl).crossFade().placeholder(defResourceId).transform(transformations).into(view);
        return this;
    }
    ...
    public ViewHolder setOnClickListener(int viewId, FrameAdapter.OnAdapterItemChildClickListener listener) {
        View view = getView(viewId);
        listener.position = mDataPosition;
        view.setOnClickListener(listener);
        return this;
    }
    ...
    /**
     * Should be called during onBindItemViewData
     */
